...
# -*- coding: utf-8 -*-
"""
Run src_v14 per-class dimension + coverage-guided adaptive branch with audit.
...
def resolve_default_paths(dataset: str, src_v14_file: str | None):
    ...
    out_dir = project_root / 'scripts' / f'results_src_v14_perclass_dim_floor_coverage_audit_{dataset}'

    if src_v14_file:
        src_path = Path(src_v14_file)
    else:
    ...
    if src_path is None or not src_path.exists():
        raise FileNotFoundError('Cannot find src_v14 algorithm file. Please pass --src-v14-file explicitly.')
...
def run_audit(mod, dataset='chameleon', data_base=None, out_dir=None, dim_candidates=None, num_splits=10):
    ...
    write_csv(out_dir / 'split_summary_src_v14_perclass_dim_floor_coverage_audit.csv', split_rows)
    write_csv(out_dir / 'test_node_predictions_all_src_v14.csv', node_rows)
    write_csv(out_dir / 'misclassified_test_nodes_src_v14.csv', mis_rows)
    write_csv(out_dir / 'branch_subspace_summary_src_v14.csv', branch_rows)
    write_csv(out_dir / 'branch_member_rows_src_v14.csv', member_rows)
    write_csv(out_dir / 'dim_search_trace_src_v14.csv', dim_trace_rows)
    write_csv(out_dir / 'oof_confusion_and_branch_debug_src_v14.csv', oof_rows)
    write_csv(out_dir / 'extra_branch_effect_summary_src_v14.csv', extra_effect_rows)
    write_csv(out_dir / 'extra_branch_rewrite_nodes_src_v14.csv', extra_rewrite_rows)
    write_csv(out_dir / 'pairwise_branch_geometry_src_v14.csv', branch_pair_rows)

    (out_dir / 'run_summary_src_v14_perclass_dim_floor_coverage_audit.json').write_text(
...
def main():
    parser = argparse.ArgumentParser(description='Run src_v14 per-class dimension coverage audit.')
    ...
    project_root, drive_root, default_data_base, default_out_dir, src_path = resolve_default_paths(args.dataset, args.src_v14_file)
    data_base = Path(args.data_base) if args.data_base else default_data_base
    out_dir = Path(args.out_dir) if args.out_dir else default_out_dir

    mod = import_module_from_path(src_path, 'src_v14_algo')
    print(f'Project root: {project_root}')
    print(f'Drive root:   {drive_root}')
    print(f'Src_v14 file: {src_path}')
