"""
Agent E3: GL-SRR 多随机种子 + 多数据集 (另一组种子 + Hessian attention)
=======================================================================
在 E2 基础上，使用 Hessian attention（E1 的改进），
验证 Hessian attention 在多数据集上是否稳定优于普通内积 attention。
种子：[10, 20, 30, 40, 50]
"""
...
LOG_PATH = os.path.join(OUT_DIR, "e3_output.txt")
log_lines = []
...
class HessianGLSRRNet(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_classes,
        ...
        self.layer1  = HessianAttentionLayer(in_dim, hidden_dim, num_heads, subspace_dim,
                                              eta, lambda_lap, lambda_sparse, eps)
        self.layer2  = HessianAttentionLayer(hidden_dim, hidden_dim, num_heads, subspace_dim,
                                              eta, lambda_lap, lambda_sparse, eps)
...
def run_dataset(ds_name, cfg, device, seeds):
    ...
    log(f"{'='*50}")
    features, labels, adj, lap, train_idx, val_idx, test_idx = load_dataset(ds_name, DATA_DIR)
...
def main():
    ...
    log(f"=== Agent E3: Hessian-GL-SRR 多种子多数据集  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===")
    log("Hessian attention + 多种子[10,20,30,40,50] + Cora/Citeseer/Pubmed\n")

    cfg = dict(hidden_dim=64, num_heads=7, subspace_dim=16, eta=0.5, eps=0.5,
    ...
    log("\n" + "="*60)
    log("Agent E3 - Hessian-GL-SRR 多种子多数据集结果汇总")
    log("="*60)
    log(f"  {'Dataset':<12} {'Val mean±std':<22} {'Test mean±std':<22}")
    log(f"  {'-'*56}")
    for ds, (vm, vs, tm, ts) in summary.items():
        log(f"  {ds:<12} {vm:.4f}±{vs:.4f}          {tm:.4f}±{ts:.4f}")

    res_path = os.path.join(OUT_DIR, "e3_result.txt")
