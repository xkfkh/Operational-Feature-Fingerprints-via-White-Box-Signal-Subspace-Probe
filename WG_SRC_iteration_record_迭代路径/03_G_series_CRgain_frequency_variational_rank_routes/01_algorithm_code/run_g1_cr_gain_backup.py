...
Layer forward pass (proximal gradient unrolling):
  ...
  2. Compute coding rate R_k for each hop
  3. w_k = softmax(R_k / tau)  -- tau is learnable temperature
  4. Gradient step: H_half = H + eta * sum_k w_k * grad_R_k - eta * lambda_lap * sum_k L @ Z_k
...
class CRGainHopLayer(nn.Module):
    ...
    Gradient step:
      H_half = H + eta * sum_k w_k * grad_R(Z_k) @ W_k^T
                 - eta * lambda_lap * sum_k L @ Z_k @ W_k^T
    ...
    def __init__(self, in_dim, out_dim, subspace_dim, num_hops=2,
        ...
        self.out_proj = nn.Linear(subspace_dim, out_dim, bias=False)
    ...
    def forward(self, H, hop_feats, L):
        ...
        lap_contrib  = torch.zeros_like(H)
        ...
        for k in range(K + 1):
            ...
            # Laplacian regularization: lambda_lap * L @ Z_k @ W_k^T
            LZ = L @ Z_list[k]                                 # [n, subspace_dim]
            lap_contrib = lap_contrib + LZ @ self.W[k].weight  # [n, in_dim]

        H_half_in = H + self.eta * grad_contrib - self.eta * self.lambda_lap * lap_contrib

        # Project to out_dim via weighted sum of Z_k
        Z_agg = sum(w[k] * Z_list[k] for k in range(K + 1))   # [n, subspace_dim]
        H_half = self.out_proj(Z_agg) + (
            self.out_proj(self.W[0](H_half_in - H))
            if self.in_dim != self.out_dim
            else torch.zeros(H.shape[0], self.out_dim, device=H.device)
        )
        # Simpler: just use projected aggregation + residual correction
        H_half = self.out_proj(Z_agg)
...
def run_experiment(name, cfg):
    ...
    for epoch in range(1, epochs + 1):
        ...
        # MCR2 loss on final layer output
        ...
        if epoch % 50 == 0:
            ...
            log(f"  Layer2 tau={tau2:.3f} | hop_w={w2_str} | R={R2_str}")
