...
def infer_node_type(P: np.ndarray) -> List[str]:
    out = []
    for row in P:
        R = row[0]
        L = row[1] + row[2] + row[3] + row[6] + row[7]
        H = row[4] + row[5] + row[8]
        two = row[2] + row[7]
        one = row[1] + row[6]
...
def plot_E_error_shift(ds: str, shift_df: pd.DataFrame, out_dir: Path):
    ...
    plt.yticks(y, df["block_label"].tolist())
    plt.xlabel("Delta_b = P_b_wrong - P_b_correct")
...
def analyze_one_split(graph, dataset: str, split_id: int, seed: int, grid_name: str, out_dir: Path, max_barcode_nodes: int):
    ...
    R = I["I_X"]
    L = I["I_PrX"] + I["I_Pr2X"] + I["I_Pr3X"] + I["I_PsX"] + I["I_Ps2X"]
    H = I["I_XminusPrX"] + I["I_PrXminusPr2X"] + I["I_XminusPsX"]
    HHI = H / (L + EPS)
    ...
    mechanism_type = infer_node_type(P)
    ...
    for c in classes:
        ...
        Pc = P[use]
        R_c = safe_mean(Pc[:, 0])
        L_c = safe_mean(Pc[:, 1] + Pc[:, 2] + Pc[:, 3] + Pc[:, 6] + Pc[:, 7])
        H_c = safe_mean(Pc[:, 4] + Pc[:, 5] + Pc[:, 8])
