...
# -*- coding: utf-8 -*-
"""
Run src_v12 classgated geometry-coupled safe adaptive-branch PCA and record:
...
Place under project/scripts and run:
    python run_src_v12_classgated_geometry_safe_branch_with_audit.py
"""
...
def discover_project_root(start: Path) -> Path:
    ...
    for p in [start] + list(start.parents):
        if (p / 'scripts').exists() and (p / 'src_v12').exists():
            return p
    raise FileNotFoundError('Cannot locate project root containing scripts and src_v12')
...
def resolve_default_paths(dataset: str, src_v12_file: str | None):
    ...
    out_dir = project_root / 'scripts' / f'results_src_v12_classgated_geometry_safe_branch_audit_{dataset}'

    if src_v12_file is not None:
        src_v12_path = Path(src_v12_file)
    else:
        src_v12_path = find_first_existing([
            project_root / 'src_v12' / 'algo1_multihop_pca_classgated_geometry_safe_branch_src_v12.py',
            project_root / 'src_v12' / 'algo1_multihop_pca_classgated_geometry_safe_branch.py',
        ])
    if src_v12_path is None or not src_v12_path.exists():
        raise FileNotFoundError('Cannot find src_v12 algorithm file in src_v12. Please pass --src-v12-file explicitly.')

    return project_root, drive_root, data_base, out_dir, src_v12_path
...
def run_audit(mod, dataset: str, data_base: Path, out_dir: Path, dim_candidates: List[int], num_splits: int):
    ...
    write_csv(out_dir / 'split_summary_src_v12_classgated_geometry_safe_branch_audit.csv', split_rows)
    write_csv(out_dir / 'test_node_predictions_all_src_v12.csv', node_rows)
    write_csv(out_dir / 'misclassified_test_nodes_src_v12.csv', misclf_rows)
    write_csv(out_dir / 'branch_subspace_summary_src_v12.csv', branch_summary_rows)
    write_csv(out_dir / 'branch_member_rows_src_v12.csv', branch_member_rows)
    write_csv(out_dir / 'oof_confusion_and_branch_debug_src_v12.csv', oof_rows)
    write_csv(out_dir / 'calibration_class_debug_src_v12.csv', calib_class_rows)
    write_csv(out_dir / 'calibration_pair_debug_src_v12.csv', calib_pair_rows)
    write_csv(out_dir / 'root_calibration_rewrite_nodes_src_v12.csv', calibration_rewrite_rows)
    write_csv(out_dir / 'extra_branch_effect_summary_src_v12.csv', extra_branch_effect_rows)
    write_csv(out_dir / 'extra_branch_rewrite_nodes_src_v12.csv', extra_branch_rewrite_rows)
    ...
    (out_dir / 'run_summary_src_v12_classgated_geometry_safe_branch_audit.json').write_text(
    ...
    print("\nWrote:")
    for name in [
        'misclassified_test_nodes_src_v12.csv',
        'branch_subspace_summary_src_v12.csv',
        'calibration_class_debug_src_v12.csv',
        'calibration_pair_debug_src_v12.csv',
        'extra_branch_effect_summary_src_v12.csv',
        'extra_branch_rewrite_nodes_src_v12.csv',
    ]:
        print(f"  {out_dir / name}")


def main():
    parser = argparse.ArgumentParser(description='Run src_v12 classgated-geometry-safe branch model with full audit.')
    ...
    parser.add_argument('--out-dir', type=str, default=None,
                        help='Output directory. Default: project/scripts/results_src_v12_classgated_geometry_safe_branch_audit_<dataset>')
    parser.add_argument('--src-v12-file', type=str, default=None,
                        help='Path to src_v12 algorithm file. Default: auto-discover under project/src_v12')
    parser.add_argument('--num-splits', type=int, default=10)
    parser.add_argument('--dims', type=int, nargs='*', default=[16, 24, 32, 48, 64])
    args = parser.parse_args()

    project_root, drive_root, default_data_base, default_out_dir, src_v12_path = resolve_default_paths(args.dataset, args.src_v12_file)
    ...
    mod = import_module_from_path(src_v12_path, 'src_v12_algo_audit')
